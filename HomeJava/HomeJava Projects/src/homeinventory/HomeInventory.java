...
//			newButton.setText("catch");
	...
	private void buttonSettings() {
		...
		previousButton.setToolTipText("Previous Current Item");
		...
		nextButton.setToolTipText("Next Current Item");
		...
		printButton.setToolTipText("Print Current Item");
...
//			outputFile.println("Ravi ranjan yadav");
			outputFile.close();
		} catch(Exception e) { }
		System.exit(0);
	}
	
	// Button Action Performed functionalities
	...
	private void checkSave() {
		...
		if(!myInventory[currentEntry-1].description.equals(itemTextField.getText()))
			...
			edited = true;
		if(edited) {
			if(JOptionPane.showConfirmDialog(null,"You have edited this item. Do you want to save the changes?","Save Item",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_NO_OPTION)
				saveButton.doClick();
		}
	}
	...
	private void locationComboBoxActionPerformed(ActionEvent e) {
		...
		if(locationComboBox.getItemCount()!=0) {
			for(int i = 0; i < locationComboBox.getItemCount(); i++)  {
				if(locationComboBox.getSelectedItem().toString().contentEquals(locationComboBox.getItemAt(i).toString())) {
...
class InventoryDocument implements Printable {
	public int print(Graphics g, PageFormat pf, int pageIndex) {
		...
		int i, iEnd;
		// decide what goes on each page and draw it
		// header
